...
function resolveFireweaveWebCredentials(env: Record<string, string | undefined>): {
  ...
  const apiUrl = (
    env.PUBLIC_FW_API_URL ??
    env.PUBLIC_FW_ATTEST_URL ??
    env.VITE_FW_API_URL ??
    env.VITE_FW_ATTEST_URL ??
    env.FW_API_URL ??
    env.FW_ATTEST_URL ??
...
export async function makeDevProvider(): Promise<FireweaveWebClient> {
  fwClient = await initFireweave({
    mode: 'local',
    local: { controlPoints: { "user-theme": true } },
    context: { targetingKey: resolveDeviceTargetingKey() },
  });
  return fwClient;
}
...
/** Sign-out: re-prefetch under the persisted device key so the next visitor does not inherit the previous bucket. */
export async function resetFireweaveUser(): Promise<void> {
  await reloadFireweaveFlags(resolveDeviceTargetingKey());
}
