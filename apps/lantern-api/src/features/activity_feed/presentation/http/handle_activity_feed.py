...
from src.features.activity_feed.application.get_activity_feed import get_activity_feed
...
def handle_activity_feed(identity, telemetry, headers) -> tuple[int, bytes]:
    ...
    try:
        body = get_activity_feed(user)
        elapsed_ms = (time.perf_counter() - started) * 1000
        telemetry.record(
            "activity.feed.latency_ms",
            elapsed_ms,
            attrs={"plan": plan},
        )
        return _json(body)
        ...
        telemetry.increment(
            "activity.feed.request.server_error",
            attrs={"plan": plan},
        )
        return _json({"error": "activity unavailable"}, 500)
