...
if [ -f "$REPO/apps/bench-cmd/main.ts" ]; then
  echo "→ bench compose up --name demo1 --bucket local --env dev"
  exec bun "$REPO/apps/bench-cmd/main.ts" compose up --name demo1 --bucket local --env dev
fi
