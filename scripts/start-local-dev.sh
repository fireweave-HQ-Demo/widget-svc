#!/usr/bin/env bash
set -euo pipefail
# Start sample_skills_git local/dev
...
if [ -f "$REPO/apps/bench-cmd/main.ts" ]; then
  echo "→ bench compose up --name sample_skills_git --bucket local --env dev"
  exec bun "$REPO/apps/bench-cmd/main.ts" compose up --name sample_skills_git --bucket local --env dev
fi
