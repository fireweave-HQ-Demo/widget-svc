#!/usr/bin/env bash
set -euo pipefail
# Start dd_e2e local/dev
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
REPO="$(cd "$ROOT/../../.." && pwd)"
if [ -f "$REPO/apps/bench-cmd/main.ts" ]; then
  echo "→ bench compose up --name dd_e2e --bucket local --env dev"
  exec bun "$REPO/apps/bench-cmd/main.ts" compose up --name dd_e2e --bucket local --env dev
fi
