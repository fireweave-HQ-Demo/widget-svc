#!/usr/bin/env bash
set -euo pipefail
# Start stg_polyglot remote/prod
...
echo "→ bench compose up --name stg_polyglot --bucket remote --env prod"
exec bun "$CMD" compose up --name stg_polyglot --bucket remote --env prod
