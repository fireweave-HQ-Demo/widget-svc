...
if [ -f "$REPO/apps/bench-cmd/main.ts" ]; then
  echo "→ bench compose up --name demo1 --bucket remote --env prod"
  exec bun "$REPO/apps/bench-cmd/main.ts" compose up --name demo1 --bucket remote --env prod
fi
