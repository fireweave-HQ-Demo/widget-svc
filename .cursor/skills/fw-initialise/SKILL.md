...
| --------- | ------------------------------------- | ---------------------------------------------------------------- |
| `stable`  | the registry's default tag (`latest`) | every normal repo — install BARE, no version range               |
| `staging` | the `staging` tag                     | dogfooding the next SDK line before it promotes                  |
| `dev`     | a LOCAL path (`workspace:*`)          | SDK co-development only, behind `sdkDev` / `FIREWEAVE_SDK_DEV=1` |

**The staging tag is named `staging`, and so is the prerelease identifier.** A
staging release is `<major>.<minor>.<patch>-staging.<N>` — e.g. `2.3.0-staging.1`
— published under the npm dist-tag `staging`. Install it by TAG, never by
version:

```
bun add @fireweaveai/server-sdk@staging      # or pnpm/yarn/npm/deno equivalent
```

Naming the tag matters because the alternative is pinning a literal staging
...
**Python is the exception, and it is a hard one.** PEP 440 admits only `a` / `b`
/ `rc` prerelease segments plus `.devN`, so `2.3.0-staging.1` is not a valid
PyPI version at all — pip cannot parse it. A `fireweave` staging release spells
itself in PEP 440 (`2.3.0rc1` or `2.3.0.dev1`) and is installed with
`--pre`; PyPI has no dist-tags to name. Record the channel as `staging` all the
same — the channel is FireWeave's vocabulary, not the registry's.
