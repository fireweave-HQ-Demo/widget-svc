...
| **5 — Scaffold `fw-tracker/` + `.fireweave/`** | Empty `fw-tracker/` const tree at the idiomatic path — **`ts-server` / `web` only**, because a directory-of-modules is the TS idiom and nothing else's. **Every other surface gets a single tracker MODULE, generated in Step 4 from its own template**: `python` → `harness/python/fw_tracker.py.tpl`, `java` → `FwTracker.java.tpl`, `swift` → `FwTracker.swift.tpl`, `go` → `fw_tracker.go.tpl`, `rust` → `fw_tracker.rs.tpl`. Do not overwrite any of them with an empty tree. **Record each surface's tracker on that surface's own entry — `surfaces[].trackerPath` (or `rolloutReady.harnesses[].trackerPath`) — never only the repo-global `rolloutReady.trackerPath`.** The repo-global pair (`trackerPath` / `webTrackerPath`) models exactly TWO surfaces; a repo with a third had its stamp checked against another surface's tracker file. Set the legacy singular as well when the repo has a ts-server surface, for readers not yet on the per-surface field. An unrecorded tracker downgrades a stamp BLOCK to a warn in `assert_dev_checklist` check 4. `.fireweave/PROVIDERS.md`. **Create NO `rollout-ready/`, NO `changelog/`, NO `_archive/`, and NO `config.json`** — see _What initialise puts in git_ above; manifests and stamps are server-owned and an empty directory is just a second place a future reader looks. Ensure `.fireweave/.gitignore` covers `deploy-beacon.env.local`, `local.json`, `.cache/`, `.queue/`, `.lock` (the provision + queue tools write these lines — re-check if missing; **never** add an ignore rule that would let a `git clean` sweep `.queue/` unremarked, and never tell a user to delete `.queue/` as a cache remedy). Also write `.fireweave/hooks/rollout-build-gate.mjs` (see **Build-gate script** below) and `.fireweave/hooks/rollout-build-gate.sh` wrapper. |
...
**Channel, and the per-surface pin.** Record what each surface actually resolved
into `project.json` `surfaces[].sdk` as `{ name, version?, channel }`:

| `channel` | Resolves from                   | Use                                                              |
| --------- | ------------------------------- | ---------------------------------------------------------------- |
| `stable`  | the registry's default dist-tag | every normal repo — install BARE, no version range               |
| `staging` | the pre-release dist-tag        | dogfooding the next SDK line before it promotes                  |
...
at whatever line was current the day it was written; dist-tags are the source of
