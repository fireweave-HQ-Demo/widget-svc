...
3. **Rewrite** — provider wiring, then identity, then call sites, then anchors, then per-surface tracker paths.
...
### 3e. Per-surface tracker paths (applies to EVERY repo, not just rewritten ones)

`.fireweave/project.json` used to record trackers in two repo-global slots —
`rolloutReady.trackerPath` and `rolloutReady.webTrackerPath`. That is a
**two-surface model**, from before FireWeave supported seven. On a repo with a
third surface the old resolver handed that surface the ts-server tracker: not a
missing path, a **wrong** one. The stamp gate then reads a file that does not
contain the surface's stamp and reports it missing.

Fix it by recording each tracker on its own surface entry. Do this **even when
the repo needed no call-site rewrite** — an already-correct repo with three
surfaces still carries the bad mapping.

**Read** the repo's surfaces and their tracker files, then **write** the path
onto each surface entry in `.fireweave/project.json`:
  ...
  "surfaces": [
    {
      ...
      "trackerPath": "src/fw-tracker/index.ts",
    }, // ts-server
    {
      "surfaceId": "sfc_…",
...
`rolloutReady.harnesses[].trackerPath` is accepted identically if the repo uses
that shape.

**Leave the legacy pair in place.** `trackerPath` / `webTrackerPath` are still
read as a fallback, and deleting them buys nothing while breaking any reader not
yet on the per-surface field. Add, do not replace.

**Do not invent a path.** Record only a tracker file that actually exists — a
recorded path that does not resolve is worse than an absent one, because the
gate stops falling back to its conventional locations. If a surface has no
tracker module, scaffold one from the initialise skill's
`harness/<surface>/fw_tracker.*` template first, or leave that surface's entry
without the field.

**Report it.** Name the surfaces you recorded in the step-5 `notes` — this is a
correctness fix a reviewer cannot see from the diff alone.
